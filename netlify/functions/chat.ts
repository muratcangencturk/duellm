...
const handler: Handler = async (event) => {
  ...
  if (path === "/tokens") {
    ...
    try {
      ...
      const { data } = await supabase.from("token_usage").select("total_tokens").eq("user_id", uid).eq("date", today).single();
  ...
  if (uid) {
    try {
      ...
      if (data && data.total_tokens >= DAILY_LIMIT) {
        return { statusCode: 429, headers, body: JSON.stringify({ error: { message: "Daily token limit reached (20K)" } }) };
      }
    } catch {}
  }
  ...
  try {
    const response = await fetch(OPENROUTER_API_URL, {
      method: "POST",
      headers: { Authorization: `Bearer ${process.env.OPENROUTER_API_KEY}`, "HTTP-Referer": "https://duellm.netlify.app", "X-Title": "duellm", "Content-Type": "application/json" },
    ...
    if (uid && data.usage?.total_tokens) {
      const today = new Date().toISOString().split("T")[0];
      try { await supabase.from("token_usage").upsert({ user_id: uid, date: today, total_tokens: data.usage.total_tokens }, { onConflict: "user_id,date" }); } catch {}
    }
